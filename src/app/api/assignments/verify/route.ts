...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!studentId) return NextResponse.json({ error: 'Нет пользователя' }, { status: 401 })

    const { searchParams } = new URL(request.url)
    const id = searchParams.get('id')
    const assignmentId = parseInt(String(id || ''))
    if (isNaN(assignmentId)) return NextResponse.json({ error: 'Неверный ID задания' }, { status: 400 })
    ...
    if (aRes.rows.length === 0) return NextResponse.json({ error: 'Задание не найдено' }, { status: 404 })
